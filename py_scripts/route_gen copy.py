...
def create_kml_from_routes(all_routes_data, main_folder_name="Các Tuyến Đường", doc_name="Các tuyến đường được tạo tự động"):
    ...
    Args:
        ...
        main_folder_name (str): Tên thư mục chính trong KML.
    ...
    if main_folder_name not in folders:
        folders[main_folder_name] = kml.newfolder(name=main_folder_name)
    ...
    for i, route_info in enumerate(all_routes_data):
        ...
        if folder_name not in folders:
            folders[folder_name] = folders[main_folder_name].newfolder(name=folder_name)
...
if __name__ == "__main__":
    ...
    mock_routes_data = [
        {
            "LineName": "Tuyến MOCK A (HCM - Đà Lạt)",
            "Latitude1": 10.771969,
            "Longitude1": 106.702083,
            "Latitude2": 11.940348,
            "Longitude2": 108.441961,
            "Color": "FF0000FF", # Đỏ
            "Width": 5,
            "Description": "Đây là tuyến đường MOCK đầu tiên.",
            "FolderName": "MOCK Tuyến Đường Chính"
        },
        {
            "LineName": "Tuyến MOCK B (HN - Hạ Long)",
            "Latitude1": 21.027763,
            "Longitude1": 105.841177,
            "Latitude2": 20.957580,
            "Longitude2": 106.945248,
            "Color": "FF00FF00", # Xanh lá
            "Width": 3,
            "Description": "Đây là tuyến đường MOCK thứ hai.",
            "FolderName": "MOCK Tuyến Đường Phụ"
        },
        {
            "LineName": "Tuyến MOCK C (HN Hồ Gươm)",
            "Latitude1": 21.037191,
            "Longitude1": 105.845892,
            "Latitude2": 21.030588,
            "Longitude2": 105.850931,
            "Color": "FFFF0000", # Xanh dương
            "Width": 2,
            "Description": "Tuyến đi bộ MOCK quanh Hồ Gươm.",
            "FolderName": "MOCK Tuyến Đi Bộ"
        }
    ]
    ...
    if all_generated_routes_data:
        ...
        if kml_content:         
             print(json.dumps({
                ...
                "fileName": "route_kml_generated.kml",
                "message": "Đã tạo KML thành công với các tuyến đường đã xử lý."
            }))

        else:
            result = {
                "status": "error",
                "message": "Không thể tạo nội dung KML."
            }
        ...
        result = {
            "status": "error",
            "message": "Không có tuyến đường nào được xử lý thành công để tạo KML."
        }
        print(json.dumps(result))
        sys.exit(1)